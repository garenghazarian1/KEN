...
2) ${stores[1].name} — ${stores[1].street}, ${stores[1].city}. Directions: ${getGoogleMapsUrl(stores[1])}. Phone ${stores[1].phone}, WhatsApp ${stores[1].whatsapp}.`,
  ...
  },
  {
    ...
    questions: ["service", "treatment", "hair", "nail", "facial", "barber", "solarium", "what do you do", "menu"],
    answer:
      "Ordinary service names, prices, and durations come only from the live catalog on our Services page (admin API). Tell the guest you can look up any specific service and its current priceLabel from that catalog — do not invent a service list from memory. For limited-time promotional packages (August offers, Mermaid / Glow Up / Princess Hands, Hot Tuesday), use the ACTIVE CAMPAIGN context when present — do not invent packages.",
    actions: [],
  },
  {
    id: "campaign_offers",
    ...
    answer:
      "Limited-time promotional packages and Hot Tuesday rules come only from the ACTIVE CAMPAIGN context when present (source: campaigns.js / /offers). Quote package promo prices and inclusions exactly from that block. If no active campaign context is provided, say there is no confirmed promo right now and point the guest to /offers or WhatsApp. Do not invent packages. Hot Tuesday (20% off every service) excludes the August Special Packages. Ordinary à-la-carte services still use the live catalog.",
    actions: [],
  },
  {
    id: "opening_hours",
